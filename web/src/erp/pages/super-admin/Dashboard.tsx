...
import { AdminLayout, StatCard } from "@/erp/components/layouts";
import { useGetPlatformStats, useListSchools, useApproveSchool, useSuspendSchool } from "@/erp/api-client";
            ...
            <div className="flex justify-between items-center mb-6">
              ...
              <div className="flex gap-2">
                <button onClick={() => setActiveChart("schools")}
                  className={`px-4 py-1.5 rounded-lg text-sm font-bold transition-all duration-200 ${activeChart === "schools" ? "erp-button-gradient" : "bg-muted text-muted-foreground hover:bg-muted/80"}`}>
                  Schools
                </button>
                <button onClick={() => setActiveChart("revenue")}
                  className={`px-4 py-1.5 rounded-lg text-sm font-bold transition-all duration-200 ${activeChart === "revenue" ? "erp-button-gradient" : "bg-muted text-muted-foreground hover:bg-muted/80"}`}>
                  Revenue
                </button>
              </div>
            </div>
              ...
              {schoolsData?.schools?.length ? (
                schoolsData.schools.map((s: any) => (
                  <div key={s.id} className="p-4 border border-border rounded-xl hover:border-primary/50 transition-colors">
                    ...
                    <div className="flex gap-2">
                      <Button size="sm" className="flex-1 text-xs font-bold rounded-lg erp-button-gradient border-none" onClick={() => handleApprove(s.id)} disabled={approveMutation.isPending}>
                        <CheckCircle2 className="w-3.5 h-3.5 mr-1" /> Approve
                      </Button>
                      <Button size="sm" variant="outline" className="flex-1 text-xs font-bold rounded-lg text-destructive hover:text-white border-destructive/20 hover:bg-destructive transition-colors" onClick={() => handleSuspend(s.id)} disabled={suspendMutation.isPending}>
